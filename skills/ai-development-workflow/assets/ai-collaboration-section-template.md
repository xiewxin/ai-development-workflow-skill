...
### 多分段摘要（條件式，兩段以上時取代逐段重複清單）

同一計畫有兩個以上已完成分段時，使用下表逐段保留完整聚合證據；不得加總不同分段的 baseline、耗時、節省工時或比例。計量 ID 必須完整輸出，不得用省略號、前後綴或短碼代替；計量模式與資料來源必須獨立填寫，不得併入範圍後省略。只有存在異常、非完整覆蓋、負值、特殊不可比原因或不同歸因邊界的分段，才在表後補充詳情。

| 分段／ID | 範圍 | 計量模式與資料來源 | PERT 基準、鎖定／指紋 | AI 耗時／階段摘要 | 節省工時／比例 | 覆蓋度／可信度 | 異常／混入工作 | 清理結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| {{INPUT:分段名稱／完整 ID，不得截斷}} | {{INPUT:full_requirement｜remaining_delivery}} | {{INPUT:session｜ActivityWatch；逐段必填}} | {{INPUT:五階段 PERT、總秒數、鎖定時間／指紋；無效時寫原因}} | {{INPUT:總秒數／五階段秒數}} | {{INPUT:本段節省秒數／比例；不可比時寫原因}} | {{INPUT:complete｜partial｜unknown／中｜低及依據}} | {{INPUT:anomalies／mixed_work 與影響}} | {{INPUT:回讀對帳與 delete 結果}} |
