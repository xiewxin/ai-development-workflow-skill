...
本文件中的「計時資料源」是 session 或 ActivityWatch。既有 CLI 的 `--provider` 只是相容參數名稱，不表示外部工作流 Provider，也不參與 Skill 編排選用。
...
- 同一計畫存在兩個以上已完成分段時，優先使用一張多分段摘要表集中呈現；每列仍保留分段／完整 ID、範圍、獨立的計量模式與資料來源、PERT 基準與鎖定／指紋、AI 耗時與階段摘要、節省工時／比例、覆蓋度／可信度、異常／混入工作及清理結果。計量 ID 是回讀對帳識別，不得以省略號、前後綴或短碼截斷；也不得把模式／資料來源合併到容易省略的範圍描述，或為壓縮篇幅刪除不可比原因、聚合多段數值。
- 多個分段共用的歸因限制只寫一次；只有某段存在異常、`partial`／`unknown`、負值、特殊不可比原因或不同歸因邊界時，才在摘要表後展開該段詳情。單一分段仍使用既有欄位清單，不強制建立表格。
...
- 多分段摘要表只是呈現壓縮，不改變每段的獨立 baseline、指紋、覆蓋度、計算或清理生命週期；表內資訊不足以重建上述欄位時，必須補充該段詳情，不得省略。
