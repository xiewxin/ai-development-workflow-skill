...
create_valid_repo() {
    ...
    printf '%s\n' \
        ...
        '完整流程參考[參考計時](references/reference-timing.md)。' \
        '' \
        '字面格式範例：`[優先級][類型] 標題`。' \
        '' \
        '### [P1][功能正確性] 虛構審查範例' \
...
measure_symlink_root="$(new_case measure-symlink)"
measure_external_path="${TEST_ROOT}/measure-external.py"
measure_external_content='external_measure_marker'
printf '%s\n' "${measure_external_content}" > "${measure_external_path}"
rm "${measure_symlink_root}/skills/ai-development-workflow/scripts/measure.py"
ln -s "${measure_external_path}" "${measure_symlink_root}/skills/ai-development-workflow/scripts/measure.py"
expect_symlink_fail \
    "參考計時腳本符號連結" \
    "${measure_symlink_root}" \
    "skills/ai-development-workflow/scripts/measure.py" \
    "${measure_external_path}" \
    "${measure_external_content}"

measure_home_path_root="$(new_case measure-home-path)"
measure_home_path='/''Users''/example-user/work/measure.py'
printf '%s\n' "${measure_home_path}" >> "${measure_home_path_root}/skills/ai-development-workflow/scripts/measure.py"
expect_fail \
    "參考計時腳本含本機絕對路徑" \
    "${measure_home_path_root}" \
    "本機家目錄路徑" \
    "${measure_home_path}"
